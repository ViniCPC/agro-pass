...
function makeDbMessage(role: 'USER' | 'MODEL', content: string, createdAt = new Date()) {
  return { id: 'msg-1', telegramUserId: TELEGRAM_ID, role, content, createdAt };
}
  ...
  describe('chat — histórico de conversa', () => {
    ...
    it('converte mensagens do banco para formato Content do Gemini', async () => {
      findMany.mockResolvedValue([
        makeDbMessage('MODEL', 'olá!', new Date('2026-05-02T10:01:00Z')),
        makeDbMessage('USER', 'oi', new Date('2026-05-02T10:00:00Z')),
      ]);
      mockSendMessage.mockResolvedValue(makeGeminiResponse('ok'));
